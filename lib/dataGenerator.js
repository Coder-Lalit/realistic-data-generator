...
function generateRealisticData(numFields, numObjects, nestingLevel, numRecords, nestedFields, excludeEmoji = false) {
    const records = [];
    for (let i = 0; i < numRecords; i++) {
        records.push(generateObject(numFields, numObjects, nestingLevel, nestedFields, excludeEmoji));
    }
    return records;
}
...
function generateObject(numFields, numObjects, nestingLevel, nestedFields = 3, excludeEmoji = false) {
    ...
    for (let i = 0; i < numFields; i++) {
        ...
        obj[fieldName] = validateAndCleanFieldValue(fieldName, rawValue, fieldType);
    }
    ...
    if (nestingLevel > 0) {
        for (let i = 0; i < numObjects; i++) {
            ...
            if (nestingLevel > 1) {
                ...
                obj[objectName] = generateObject(nestedFields, numObjects, nestingLevel - 1, nestedFields, excludeEmoji);
            } else {
...
function validateAndCleanFieldValue(fieldName, fieldValue, fieldType) {
...
module.exports = { generateRealisticData, generateObject, generateSimpleObject };
